...
class Logger:
    ...
    def log(self, text, log_type="DEBUG", exception=None):
        ...
        if log_type not in valid_log_types:
            raise ValueError(
                'Invalid log_type "{}", must be either {}'.format(
                    log_type, valid_log_types
                )
            )
        ...
        # Open log file
        today = date.today()
        log_file_path = r"{}/log_{}_{}_{}.txt".format(
            self.log_folder, today.year, today.month, today.day
        )
        ...
        if (
            env == "DEVELOPMENT"
            or (env == "PRODUCTION" and log_type == "ERROR")
            or env is None
        ):
