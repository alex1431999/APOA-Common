...
class QueriesIndexTests(QueryTests):
    ...
    sample_index_inserted = Index(ObjectId(), "sample index inserted", [sample_user], False)

    def setUp(self) -> None:
        super().setUp()
        self.mongo_controller.indexes_collection.insert_one(self.sample_index_inserted.to_json())

    def test_add_index_new_index(self):
        user = ObjectId()
        index = self.mongo_controller.add_index(self.sample_index.name, user, return_object=True, cast=True)

        self.assertEqual(index.name, self.sample_index.name, "the correct index was inserted")
    ...
    def test_add_index_new_user(self):
        ...
        index = self.mongo_controller.add_index(self.sample_index.name, user_new, return_object=True, cast=True)

        self.assertEqual(len(index.users), len(self.sample_index.users) + 2, "Only exactly 2 new users should have "
                                                                             "been added")
    ...
    def test_get_index(self):
        index = self.mongo_controller.get_index(self.sample_index_inserted.name, cast=True)

        self.assertIsNotNone(index, "Should have found something")
        self.assertEqual(index._id, self.sample_index_inserted._id, "The correct index should have been returned")
    ...
    def test_get_index_by_id(self):
        index = self.mongo_controller.get_index_by_id(self.sample_index_inserted._id, cast=True)
